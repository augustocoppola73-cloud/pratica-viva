...
function recordDaimokuSession(minutes){
  ...
  stats.minutes = Number(stats.minutes || 0) + Math.max(0, Number(minutes || 0));
...
function renderStats(){
  ...
  const s = getStats();
  statsBox.innerHTML = `
    <div class="stat-tile"><span>Minuti Daimoku</span><strong>${Number(s.minutes || 0)}</strong></div>
    <div class="stat-tile"><span>Sessioni completate</span><strong>${Number(s.sessions || 0)}</strong></div>
    <div class="stat-tile"><span>Giorni consecutivi</span><strong>${Number(s.streak || 0)}</strong></div>
  `;
}
...
async function finishDaimokuSession(){
  ...
  recordDaimokuSession(Math.round(durationSeconds / 60));
...
function pauseTimer(){ clearInterval(timerInterval); timerInterval = null; stopDaimokuAudio(false); updateTimerDisplay(); }
...
resetStatsBtn?.addEventListener('click', () => { if(confirm('Azzerare le statistiche di pratica?')){ localStorage.removeItem(statsKey); renderStats(); } });
