...
function renderPracticeAssistant(){
  ...
  document.getElementById('assistantOpenSuggestion')?.addEventListener('click', () => {
    openAssistantStudySuggestion(suggestion.title);
  });

  document.getElementById('assistantUseIntention')?.addEventListener('click', () => {
...
function goCompleteSessionPrayers(){
  ...
  setTimeout(() => {
    completeSessionPrayersPanel?.scrollIntoView({behavior:'smooth'});
  }, 100);
}
...
const isAndroidApk = new URLSearchParams(window.location.search).get('apk') === '1';
