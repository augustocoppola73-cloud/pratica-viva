...
function updateDailyBadge(){
  const stats = getStats();
  const todayMinutes = Number(stats.todayMinutes || 0);
  ...
  if(sub){
    if(todayMinutes === 0) sub.textContent = 'Inizia con pochi minuti di Daimoku.';
    else if(todayMinutes < target) sub.textContent = `Ti mancano ${target - todayMinutes} minuti per l’obiettivo.`;
    else sub.textContent = 'Obiettivo raggiunto. Ottimo lavoro.';
  }
}
