...
function renderPracticeAssistant(){
  ...
  const todayMinutes = (s.lastDate === dateOnlyString())? Number(s.minutesToday || 0) : 0;
  const target = getDailyTarget ? getDailyTarget() : 20;
  const theme = detectAssistantTheme();
  const suggestion = getStudySuggestionForTheme(theme || 'neutral');  let message = '';
  if(todayMinutes === 0){
    message = 'Inizia con pochi minuti. Non serve essere pronto, basta iniziare 🙏'; 
  }
  else if(todayMinutes < target){
    const remaining = target - todayMinutes;
    message = `Sei a ${todayMinutes} min. Ancora ${remaining} min per il tuo obiettivo 🔥`;
  }
  else{
  ...
  if(stats.streak === 3) message = "Ottimo inizio 🔥";
  if(stats.streak === 7) message = "Una settimana perfetta 💪";
  if(stats.streak === 30) message = "Sei una macchina 🧠";
